...
- RGB 彩色相机：已通过 OpenCV/DirectShow `RgbUvcCamera` 接入，当前电脑历史验证默认 `device_index=1`、`MJPG`、`3840x2160`、`25fps` 可预览和应用参数；P1B-8.1 起 `MJPG -> OpenCV decode -> RGB -> PNG` 明确为 `NOT SCIENTIFIC LOSSLESS`。RGB 网页预览可继续使用 MJPG latest-frame/latest encoded JPEG cache；正式 RGB scientific capture 改为独立 scientific profile，拍摄前暂停/释放 preview，打开 scientific mode，读取 actual FOURCC/source transport，只有 RAW Bayer/RGB24/BGR24 等已确认完整未压缩传输才允许保存 PNG。MJPG/JPEG/H264/H265 会返回 `RGB_SCIENTIFIC_TRANSPORT_LOSSY`，YUY2/YUYV/UYVY 记为 `uncompressed_but_chroma_subsampled` 且 strict lossless=false，unknown transport 返回 `RGB_SCIENTIFIC_LOSSLESS_UNAVAILABLE`。本轮当前环境 `manual_camera_test.py --rgb-lossless-probe` 对 `device_index=1` DirectShow 打开失败，未找到 strict-lossless mode，因此 RGB scientific capture 当前为 BLOCKED。P1B-3.7 设备选择层会尝试读取 Windows PnP/FriendlyName/VID/PID/USB serial 等信息；只有可靠映射才生成 stableId，DirectShow index 仍只是 last known location。P1B-8.1 后相机设置新增后端权威 `config/camera_settings.json`，区分 Apply、Apply & Save、Save Default、Restore Saved、Restore Default；RGB 持久化 preview requested settings，并可记录独立 `scientificProfile`，启动或重新打开相机后按 store 恢复并回读，设备身份不一致返回 `CAMERA_SETTINGS_DEVICE_MISMATCH`。
...
P1B-8.1 后正式 capture metadata 会额外记录 `settingsSource`、`settingsRestoreState`、`requestedSettings`、`actualSettings`。RGB metadata 还记录 `requestedFourcc`、`actualFourcc`、`sourcePixelFormat`、`sourceCompression`、`scientificStrictLossless`、`outputFormat=PNG` 和 `outputLossless=true`；PNG 只在 actual transport strict-lossless 通过后才保存。无后端保存配置时 `settingsSource=default` 不构成 true capture blocker；如果已保存配置恢复失败会给 readiness warning，设备 identity mismatch 则作为 `CAMERA_SETTINGS_DEVICE_MISMATCH` blocker，避免把 A 设备配置静默套到 B 设备。
...
- P1B-8.1 Camera Settings Persistence 已软件实现；RGB strict-lossless scientific capture gate 也已软件实现。后端配置文件负责 Apply/Persist/Restore/Readback 闭环，但这只是上位机本地配置持久化，不代表 RGB 或 DVP2 参数写入相机 EEPROM/UserSet。当前 RGB 真机 strict-lossless mode 未找到，True Capture 会因 `RGB_SCIENTIFIC_LOSSLESS_UNAVAILABLE` / `RGB_SCIENTIFIC_TRANSPORT_LOSSY` 阻断正式 RGB scientific capture。
