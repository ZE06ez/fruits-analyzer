...
## 2026-09-05 STM32 执行器实机验收与时序记录

- 实机结果：DAPLink VCOM `COM28`、115200-8N1 通信正常；风扇完成10s启停；现场仅接 LED3/PB9 且完成单路与联动点亮；推杆完成双向点动、全行程和端点方向性验证；HBS57H 步进电机完成22.5°点动、10rpm连续运行和20s多设备联动。
- 推杆时间：分段人工观察得到伸出有效运动约19s、缩回约20s，按当前机构单程约20s记录。固件继续保留28s单次连续动作超时，不依据一次现场观测直接缩短保护值；停止后重新启动会重置超时计时。
- 限位结论：推杆在缩回端继续同方向命令时无位移且安静，反向0.5s可立即移动，返回端点后再次安静停止，表现符合推杆内置限位。该结论不等于STM32已读取限位；PC3/PC4仍为预留，协议ACK也不提供位置/到位反馈。
- 发现问题：DAPLink VCOM 连续突发发送风扇/LED/推杆短帧后紧跟电机AA55 MOVE时，现场出现过MOVE无ACK且步进未启动。采用逐条等待ACK、同时检查STATUS运动状态、失败最多重试3次后，20s整体联动通过。
- 修改文件：`stm32/硬件实测记录_2026-09-05.md`、推杆协议/交接/开发建议/打包说明、`ctrl_door.h`注释、`docs/PROJECT_CONTEXT.md`与本变更日志。
- 是否影响原有功能：不改变GPIO、命令字、推杆方向或28s超时数值；本次只统一现场事实、保护语义和上位机可靠通信要求。
