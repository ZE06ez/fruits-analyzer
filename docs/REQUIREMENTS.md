...
| 样品台硬件模式不得伪造成功 | 已实现 | P1B-7 `sample_stage.py` 定义 `UnimplementedSampleStage`，硬件模式没有真实 adapter 时返回 `hardware_not_implemented`；simulation 只用于 unittest/离线软件编排验证，不会自动 fallback 为硬件成功 |
...
| 样品采集应支持样品台多角度旋转拍摄设置 | 已实现/部分实现 | 主程序可设置期望角度间隔、起始角度、方向和闭合补拍；后端生成 `captureRotationPlan` 并写入 metadata；P1B-7 复用该 plan 做 MultiView 软件编排；当前真实样品台硬件未实现 |
