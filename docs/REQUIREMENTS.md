...
| 新建样品应区分正常检测和训练数据采集 | 已实现 | `sample_mode=inspection/training_capture` 写入 SessionState 和 `metadata.json`。正常检测继续显示模型状态并使用 Production/Default/generic 规则；训练数据采集隐藏检测模型区块，允许直接输入新的 Fruit Type / Variety，不因为没有模型而阻塞，也不生成假预测 |
...
| 当前 firmware 不支持的硬件能力不得伪造成成功 | 已实现/架构约束 | 钨灯不再被标记为 unsupported：PB7/PB8 已确认通过 `LED_SET=0x12` 控制。仍不得伪造不存在的独立 0x13 钨灯协议、亮度闭环、门端点反馈、双钨灯验收或 RGB 正式照明映射。FAN_SET payload 为 duty 0/100；LED_SET payload 为 bitmask 0x00..0x07；DOOR_SET payload 为 0 raise/retract、1 close/extend、2 stop，ACK 只代表 command accepted；SET_ORIGIN 只表示人工对准后建立逻辑 0°；STATUS position 只作为 logical position，不标记 CL57C physical encoder verified；PPR mismatch 只报告并要求人工确认，不自动 SET_CONFIG |
| STM32 Web 硬件调试必须通过后端统一控制链 | 已实现/需实机验收 | 设备准备/电机页已提供风扇 on/off、LED3 on/off、推杆伸出/缩回/停止、滤光轮顺/逆时针相对移动、滤光轮 STOP、人工 SET_ORIGIN；光源页新增 `POST /api/device/tungsten` 和 `/api/device/tungsten/all-off` 支持 PB7/PB8 钨灯限时测试和关闭。所有 HTTP API 通过 `DeviceManager -> Stm32ControllerAdapter -> SerialService`，不新建串口服务，不调用 `manual_stm32_test.py`，不在前端映射协议 payload |
| STM32 运动与输出验证必须使用 fresh STATUS | 已实现/需实机验收 | STATUS cache 带 revision 和 `statusReceivedMonotonic`；`query_status(require_fresh=True)` 不用旧缓存冒充新状态。风扇、LED3 和钨灯命令后回读 fresh STATUS 验证 duty/mask；ACK OK 但无 fresh STATUS 不能报告 confirmed。Web 滤光轮移动默认 `maxRetries=0`，只有 post-command fresh STATUS 证明 target/position 变化、最终位置到达、motor idle/done 且 errorCode=0 才算完成。ACK OK 但 position/target 未变化失败为 `motion_not_started` 并 STOP |
...
| RGB 相机设置应区分保存配置和应用到真实相机 | 已实现 | 相机设置页提供“应用到相机”“应用并保存”“保存为默认配置”“恢复已保存配置”“恢复默认配置”；`/api/camera/rgb/apply-settings` 下发参数并回读 actual 状态，`persist=true` 时成功后写入后端被 Git 忽略的运行时 `config/camera_settings.json`；后端保存 device identity/index、width/height/fps/fourcc、auto/manual exposure、gain 和 white balance，并记录 requested/actual/settingResults；`config/camera_settings.example.json` 只作为不绑定真实设备的参考配置 |
...
| Background Reference 必须独立于 Dark/White Calibration | 已实现 | Background Reference 用于 RGB 前景分割参考；Dark Reference/White Reference 用于光谱黑白校正。两者不共用 ID，不互相替代。新建 Sample 时 metadata 独立记录 `background_reference`，且 `backgroundReferenceId` 不能等同于 `calibrationId` |
| 背景参考必须做兼容性保护 | 已实现/测试覆盖 | `BackgroundReference` 保存 image sha256、分辨率、device、cameraProfile、cameraSettings、illumination 和 stage metadata；校验失败返回 `BACKGROUND_REFERENCE_*` 错误码。参考 age 不单独失效；分辨率不一致时必须失败，不 resize、不伪造 registration |
| 背景参考分割必须输出可解释诊断 | 已实现/测试覆盖 | `BackgroundReferenceSegmenter` 输出 mask、foreground ratio、componentCount/componentAreas、selectedComponentArea、bbox、centroid、touchesImageBorder、differenceStats、qualityFlags、errorCode、timingMs、referenceId；错误码覆盖未检测、过小、过大、触边、多大连通域和背景差异过低 |
| Fruit Mask 质量评估应使用 IoU/Dice | 已实现/测量能力 | `compute_mask_iou()` 与 `compute_mask_dice()` 保留为通用 mask 质量工具，用于未来人工 mask vs 背景参考 mask 验收；当前不能声称覆盖所有水果或达到固定 Mean IoU |
...
| LED 光源开关和亮度控制 | PB7/PB8 两路钨灯 SSR 和 PB9/LED3 开关已接入 Web 手动控制；亮度控制、RGB 正式照明映射、双钨灯同时开启和硬件级安全联锁需现场验收 |
| 门控、急停、温度、电机报警状态 | 升降门、急停、故障码已有基础 API；温度和扩展报警未接入 |
| RGB 与多光谱 calibrated ROI 生产接入 | P1C-2A 已完成离线 profile 标定工具；`quality_algorithm.roi.apply_mask_to_image(registration_mode="calibrated")` 仍未接入生产特征提取，待 P1C-2B |
