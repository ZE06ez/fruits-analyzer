...
| 样品采集必须先完成设备准备流程 | 已实现/部分真实 | 前端和后端要求先完成设备检查；当前离线验证门槛为控制器/电机/光源控制。P1B-8 后 True Hardware Capture 入口通过 `DeviceManager.capture_readiness()` 动态检查 sample/output、STM32、RGB、DVP2、滤光轮、CalibrationSet、操作员确认和 SampleStage；`trueCapturePrepared` 只表示当前 plan readiness，不表示 hardware acceptance PASS |
