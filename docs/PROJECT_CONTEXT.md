...
| 滤光片转轮 | 文档建议 8-16 孔，STM32 控制 HOME/定位；代码开发配置启用 450/560/670 nm | PARTIAL/REAL VERIFIED：`stm32/` 已导入 STM32F407 固件交付工程；2026-09-05 已通过 DAPLink COM28 实测 STEP/DIR 电机可按10rpm运动并参与20s联动。上位机已有 `serial_service.py`、`hardware_controller.py`、`device_manager.py` 两字节控制层，但固件当前 AA55/短帧/ASCII 协议与上位机严格两字节协议尚未统一；DAPLink VCOM 突发多命令时现场出现过 MOVE 无 ACK/未执行，必须逐条等待 ACK/状态并重试 |
...
| 电机/驱动器 | STM32 + 步进驱动器控制滤光轮、可能的样品台/门控 | PARTIAL/REAL VERIFIED：滤光片轮、风扇、LED3和升降门已完成单项及20s联动实测。当前推杆分段实测伸出有效运动约19s、缩回约20s，端点表现符合推杆内置限位；固件28s仅为单次连续动作超时，`91 00`不代表到位，PC3/PC4外部限位仍未接入。样品旋转平台仍只有计划和UI，没有真实电机接口 |
