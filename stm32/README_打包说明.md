...
├── 协议与接线说明.md         ← 接线细节汇总
├── 硬件实测记录_2026-09-05.md ← 当前实机端口、动作时间、限位与联动问题
...
- **升降门(电动推杆)**:XY-160D 双路 H 桥第 1 路,短帧控制;当前机构实测单程约20s,28s 为单次连续动作超时保护
...
- 推杆没有位置反馈;`91 00` 只表示命令被接受。当前机构实测伸出有效运动约19s、缩回约20s,端点处表现符合推杆内置限位,但仍需外部限位实现系统级安全确认
- 多设备联动命令必须串行发送并逐条等待 ACK;现场曾出现短帧突发后电机 MOVE 帧无 ACK/未执行,确认失败时最多重试3次
